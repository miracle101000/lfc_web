...
class PamphletsUpload extends StatefulWidget {
  const PamphletsUpload({super.key});

  @override
  State<PamphletsUpload> createState() => _PamphletsUploadState();
}

class _PamphletsUploadState extends State<PamphletsUpload> {
  ...
  void initState() {
    super.initState();
    Provider.of<PamphletsProvider>(context, listen: false)
  ...
  Widget build(BuildContext context) {
    return Consumer<PamphletsProvider>(builder: (context, b, _) {
      return SizedBox(
  ...
  _function(PamphletsProvider b) async {
    ...
    if (_downloadUrl.isNotEmpty && _downloadUrl1.isNotEmpty || b.isEdit) {
      if (!b.isEdit) {
        ...
        await MongoDB.updateData(
            ...
            onDone: (_) {
              Provider.of<PamphletsProvider>(context, listen: false).isEdit = false;
              if (mounted) {
                setState(() {
                  isLoading = false;
                  _downloadUrl = '';
      ...
      if (_downloadUrl1.isEmpty) {
        await Helper.showToast("Expecting an uploaded pamphlets");
      }

      if (_downloadUrl.isEmpty && _downloadUrl1.isEmpty) {
        await Helper.showToast("Expecting an uploaded image & pamphlets");
      }
    }
  }
